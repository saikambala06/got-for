...
const api = {
  async _req(method, url, body) {
    ...
    if (!res.ok) {
      ...
      // Carry the server's actionable detail through. A 503 from a
      // misconfigured deployment says which setting is missing, and dropping
      // that leaves the user staring at a message they cannot act on.
      err.hint = data.hint || '';
      err.code = data.code || '';
