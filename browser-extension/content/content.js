...
(function () {
  ...
  let panel = new SKVKPanel();
  let studio = new TailorStudio();
  ...
  function confirmReload() {
    panel.showReloadConfirm({
      ...
      onNo: async () => { panel.renderLoading('Reloading job details…'); await loadAndRender(); }
    });
  }
  ...
  async function renderHtmlToPdf(html, fileBase) {
    ...
    // html2canvas can capture a blank canvas when the source is positioned
    // far outside the viewport (the old -99999px technique did exactly that
    // on several Chromium/ATS pages). Keep the source inside the viewport,
  ...
  async function downloadTailoredResume(resume, opts, btn) {
    ...
    try {
      // Prefer the authenticated server PDF generator. It creates a real PDF
      // vector/text document and avoids html2canvas blank-page failures on
      // strict CSP / zoomed / virtualized job boards.
      const result = await send('resumes:downloadPdf', { resumeData: resume, customOptions: opts });
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = result.filename || `${fileBase}.pdf`;
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message?.type === 'panel:toggle') {
      // The X button fully removes the panel host. A later toolbar click must
      // create a brand-new UI instance instead of trying to show a detached
      // DOM node. This keeps the extension closed on the tab until the user
      // explicitly clicks the extension icon again.
      document.querySelectorAll('#skvk-assistant-host').forEach((node) => {
        if (!panel?.host || node !== panel.host) node.remove();
      });
      if (!panel || panel.destroyed || !panel.host.isConnected) {
        panel = new SKVKPanel();
        window.__skvkAssistantPanelDestroyed = false;
      }
      if (!panel.isOpen) {
        panel.open();
        loadAndRender();
