(function () {
  ...
  function loadHtml2Pdf() {
    ...
    pdfLoader = new Promise((resolve, reject) => {
      ...
      script.onerror = () => { pdfLoader = null; reject(new Error('Could not load the PDF generator')); };
      document.head.appendChild(script);
    });
    return pdfLoader;
  }

  async function downloadTailoredResume(resume, opts) {
  ...
  async function init() {
    renderShell({ active: 'resumes', title: 'Quick Download', bodyHtml: '', apiBase: await getApiBase() });
