...
async function getConfig() {
  const { token, user, apiBase } = await chrome.storage.sync.get(['apiBase']);
  const session = await chrome.storage.local.get(['token', 'user']);
  return { apiBase: (apiBase || DEFAULT_API_BASE).replace(/\/$/, ''), token: session.token || null, user: session.user || null };
}
...
const handlers = {
  ...
  async 'auth:login'({ email, password }) {
    const { apiBase: base } = await getConfig();
  ...
  async 'settings:set'({ apiBase }) {
    const clean = String(apiBase || '').trim().replace(/\/$/, '');
    if (!/^https?:\/\//i.test(clean)) throw new Error('API base must start with http:// or https://');
    await chrome.storage.sync.set({ apiBase: clean });
  ...
  async 'resumes:downloadPdf'({ resumeId, resumeData, customOptions = {} }) {
    const { apiBase, token } = await getConfig();
    const url = resumeId ? `${apiBase}/api/resumes/${encodeURIComponent(resumeId)}/download-pdf` : `${apiBase}/api/resumes/download-pdf`;
    const res = await fetch(url, {
      ...
      body: resumeId ? undefined : JSON.stringify({ resumeData, customOptions })
    });
    if (!res.ok) {
      const body = await res.json().catch(() => ({}));
      throw new Error(body.error || `PDF download failed (${res.status})`);
    }
    ...
    return { base64: btoa(binary), filename: `SKVK_Tailored_Resume_${Date.now()}.pdf` };
  },
