/**
 * xAI (Grok) API key pool.
 *
 * Lets an operator configure more than one xAI API key so that when one
 * key's quota/rate limit is exhausted (HTTP 429), requests transparently
 * move on to the next working key instead of failing in front of the user.
 *
 * Configuration (any/all of these can be used together — the pool merges
 * and de-dupes them):
 *   XAI_API_KEY        single key (existing behaviour, still works)
 *   XAI_API_KEYS       comma or newline separated list of keys
 *   XAI_API_KEY_1..N   numbered individual keys
 *
...
function loadKeysFromEnv() {
  ...
  Object.keys(process.env)
    .filter((k) => /^XAI_API_KEY_\d+$/.test(k))
...
let instance = null;
function getKeyPool() {
  if (!instance) instance = new XaiKeyPool();
  return instance;
}

module.exports = { getKeyPool, XaiKeyPool };
