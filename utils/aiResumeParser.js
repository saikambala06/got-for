  ...
  const skills = (() => {
    ...
    return arr(skillsRaw).flatMap(s => {
      ...
      if (s && typeof s === 'object') {
        ...
        if (Array.isArray(s?.items)) return s.items.flatMap(i => typeof i === 'string' ? i.split(/[,;]/).map(t => t.trim()).filter(Boolean) : [str(i?.name, i?.skill)].filter(Boolean));
        if (Array.isArray(s?.skills)) return s.skills.map(i => str(i?.name ?? i));
        return [str(s?.name, s?.skill, s?.value, s?.label)].filter(Boolean);
      }
      return [];
    });
  })();
