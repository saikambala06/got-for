 ...
 * Groq API key pool.
 *
 * Lets an operator configure more than one Groq API key so that when one
 ...
 * and de-dupes them):
 *   GROQ_API_KEY        single key (existing behaviour, still works)
 *   GROQ_API_KEYS        comma or newline separated list of keys
 *   GROQ_API_KEY_1..N   numbered individual keys
 *
...
let instance = null;
function getKeyPool() {
  if (!instance) instance = new GroqKeyPool();
  return instance;
}

module.exports = { getKeyPool, GroqKeyPool };
